...
namespace Common
{
    ...
    //borrow from => https://github.com/Enichan/Ini
    ...
    //intB=not an int
    //doubleA = 0.238471
    //doubleB=not a double
    //doubleC = NaN

    //[test2]
    //str=normal string
    //quotstr = "quoted string"
    //wstr=    whitespace string
    //wquotstr = "      quoted whitespace string     "
    ...
    public interface ISimpleIniFile
    {
        IDictionary<string, object> TryLoadIniFileItems(string iniPath);
        string CreateKey(string sectionKey, string itemKey);
        object GetValue(IDictionary<string, object> items, string sectionKey, string itemKey);
        void SetProperties(IDictionary<string, object> items, object toBeUpdated, string modelName);
    }

    public class SimpleIni : ISimpleIniFile
    {
        #region for di extensions

        private static Lazy<SimpleIni> LazyInstance = new Lazy<SimpleIni>(() => new SimpleIni());
        public static Func<ISimpleIniFile> ResolveFile { get; set; } = () => LazyInstance.Value;

        #endregion

        public IDictionary<string, object> TryLoadIniFileItems(string filePath)
        {
            ...
            var ini = new IniFile();
            ini.Load(filePath);
            foreach (var sectionKey in ini.Keys)
            {
                var tryGetSection = ini.TryGetSection(sectionKey, out var section);
                if (tryGetSection)
                {
                    foreach (var itemKey in section.Keys)
                    {
                        var tryGetValue = section.TryGetValue(itemKey, out var item);
                        if (tryGetValue)
                        {
                            var theKey = CreateKey(sectionKey, itemKey);
                            result[theKey] = item.GetString();
                        }
                    }
                }
            }
        ...
        public string CreateKey(string sectionKey, string itemKey)
        {
            return string.Format("{0}.{1}", sectionKey, itemKey);
        }

        public object GetValue(IDictionary<string, object> items, string sectionKey, string itemKey)
        {
            var theKey = CreateKey(sectionKey, itemKey);
            items.TryGetValue(theKey, out var result);
            return result;
        }
